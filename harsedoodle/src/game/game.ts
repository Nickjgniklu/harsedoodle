...
class Game extends React.Component {
  ...
  render() {
    //this render is used by react see renderGame for gameLoop render function

    return e(
      'div',
      { className: 'centerContainer', style: { flexDirection: 'column' } },
      e('canvas', {
        id: 'canvas-main',
        width: `${2048}`,
        height: `${2048}`,
  ...
  renderBackground(context: CanvasRenderingContext2D) {
    ...
    let Yindex = Math.floor(
      (top / this.GameModel.WORLDSIZE) * this.GameModel.YTILECOUNT
    )
    ...
    let XtilePercentage =
      1 -
      ((left / this.GameModel.WORLDSIZE) * this.GameModel.XTILECOUNT - Xindex)
    let YtilePercentage =
      1 -
      ((top / this.GameModel.WORLDSIZE) * this.GameModel.YTILECOUNT - Yindex)
    ...
    let tilesinviewport =
      (this.GameModel.VIEWPORTSIZE / this.GameModel.WORLDSIZE) *
      this.GameModel.XTILECOUNT
  ...
  update(elapsedTime: number) {
    if (this.LastFrameTime != null) {
      ...
      this.GameModel.TimeSinceStartSeconds = Math.floor(
        this.GameModel.TimeSinceStart / 1000
      )
